...
> **來源**：SLO 層（見 [`docs/slo.zh-TW.md`](../slo.zh-TW.md)），目標值由 2026-09-06 實測推導
...
| 觀察 | 判斷 | 往哪走 |
|---|---|---|
| 可用性在燒 + `ServiceDown` 亮 | 真的掛了 | [`service-down.md`](service-down.zh-TW.md) |
| 延遲在燒 + `JvmGcTimeRatioHigh` 亮 | 事故 #1 的形狀 | [`gc-death-spiral.md`](gc-death-spiral.zh-TW.md) |
| 延遲在燒但 JVM 正常 | 下游或主機飽和 | [`host-saturation.md`](host-saturation.zh-TW.md) |
| 工作進度在燒但探測全綠 | **事故 #2 的形狀** | [`engine-not-progressing.md`](engine-not-progressing.zh-TW.md) |
| 三個都在燒 | 主機層問題 | [`host-down.md`](host-down.zh-TW.md) |
...
- 如果這次燒錄是**已知且可接受**的（例如刻意的維護窗口），不要調鬆
  SLO 目標來讓告警閉嘴。正確做法是記錄這次消耗，並在 `docs/slo.zh-TW.md`
  補上該不該把它排除在 SLI 之外的理由。
- 如果同一個形狀重複出現，那不是預算問題，是設計問題 —— 該修的是
  底下的服務，不是門檻。
- 目標值本身要隨實測重新校準；`docs/slo.zh-TW.md` 記錄了每個數字的來源，
