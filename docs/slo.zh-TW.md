...
4. **工作進度的歷史從 2026-09-06 重新開始。** 09-03 那 115 分鐘的缺口是從 textfile
   來源（`job="node"`）量到的。現在引擎自己提供同一個計數器（`job="trading-engine"`），
   SLI 已釘在這個來源，任何第二個來源都不可能讓預算翻倍。因此 `make slo` 從 09-06
   起算；`job="node"` 的序列在 30 天保留期內仍可在 Prometheus 查到，需要重現 09-03 的數字時用它。
