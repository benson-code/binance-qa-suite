...
- **The service now reports on itself, and the monitoring stack watches itself** — `payment-api` exposes RED metrics at `/metrics` (hand-rolled, no client library): request rate, error rate and a latency histogram whose `le="0.25"` boundary is the SLO threshold. Before this, every signal came from a black-box probe of `/health`, which reports 100% success while `/payments` returns 500 to every caller. A **dead man's switch** alert fires permanently and is routed to a heartbeat receiver: **not receiving it is the incident** — the only way to distinguish "nothing is wrong" from "the alerting pipeline is dead", which is the shape of both incidents applied to the monitoring system itself.
...
- **Observability built from real incidents** — 39 Prometheus alert rules whose thresholds are reverse-engineered from two measured production incidents · 15 runbooks with CI-enforced coverage · Alertmanager routing with four inhibition rules · one-command incident-scene preservation.
...
│   └── runbooks/                  ← 15 alert-response SOPs (CI-enforced coverage)
...
15 runbooks cover all 39 alerts. Each follows the same six sections: trigger
